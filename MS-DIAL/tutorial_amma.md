...
*Remove features based on blank information*: Check this to remove features which appeared in any of the blank samples and should be removed. Thus check this if you would like to filter the factors which appeared even in blank samples. <br>
...
-->
<!--
  これは、大多数の人がチェックを入れる項目な気がしたので、最初から（つまり規定値を）チェックが付いているといいのでは？と思ったが、3章のデモデータではブランクの指定が無かったので、やっぱりオプションでいいのかもと思いました。(でもblankを設定するなら絶対チェック付けてね、とは説明追加してもいいかも？)
-->

*Keep 'identified' metabolite features*: 'identified metabolite' means the metabolites which meet <!-- what of -->any reference.　<!-- リファレンスの何と一致していると言う？-->
<!--編集中

-->

<!-- コメント
  identifiedは現メインウィンドウではRef. matchedになっているので、'Analysis parameter setting'ウィンドウのこのidentifiedの部分、あるいは、メインウィンドウの'Peak spot navigator'ペインのRef. matchedの部分のどちらかを更新してください.>津川さん
-->
...
<!-- コメント
  annotatedは現メインウィンドウではSuggestedになっているので、'Analysis parameter setting'ウィンドウのこのannotatedの部分、あるいは、メインウィンドウの'Peak spot navigator'ペインのSuggestedの部分のどちらかを更新してください.>津川さん
-->
